...
// 编码方式： 二进制编码
// todo 设置多种交叉方式 1. 单点交叉 2. 多点交叉 3. 均值交叉
// todo 设置多种变异方式 1. 随机变异 2. 单点变异 3. 多点变异 4. 均值变异
// todo 多种选择方式 1. 轮盘赌选择 2. 随机选择 3. 锦标赛选择 4. 多目标选择
// todo 多种适应度计算方式 1. 均方误差 2. 绝对值误差 3. 相对值误差 4. 自定义适应度计算方式

namespace GA_Binary_Endode
{
	public class GeneticAlgorithm
	{
		...
		private string[][]? chromosomeMatrix; // 染色体矩阵
		private double[][]? chromosomeMatrix2; // 染色体矩阵
		...
		public void Run()
		{
			...
			for (int itIndex = 0; itIndex < iteratorNum; itIndex++)
			{
				...
				newChromosomeMatrix = Cross(newChromosomeMatrix, naturalSelectionRate);
		...
		*/
		...
		private string[][] Cross(string[][]? newChromosomeMatrix, double[] naturalSelectionRate)
		{
			...
			for (int i = 0; i < newChromosomeMatrix.Length; i++)
			{
				...
				if (rate <= crossoverRate)
				{
					...
					int j = random.Next(0, chromosomeNum); // 选择需要交叉的母本
														   // 交叉方法
														   // 单点交叉
					string[] crossChromosome = new string[K];
					for (int k = 0; k < K; k++)
					{
						int point = random.Next(1, numberOfBits - 1); // todo 随机选择交叉点 1-numberOfBits-1
						crossChromosome[k] = newChromosomeMatrix[i][k].Substring(0, point) + newChromosomeMatrix[j][k].Substring(point);
					}
		...
		private string[][] Mutation(string[][] newChromosomeMatrix, int iterations)
		{
			...
			for (int i = 0; i < chromosomeNum; i++)
			{
				for (int j = 0; j < K; j++)
				{
					...
					{
						string tmp = newChromosomeMatrix[i][j];
						int point = random.Next(0, numberOfBits);
						tmp = tmp.Substring(0, point) + (tmp[point] == '1' ? '0' : '1') + tmp.Substring(point + 1);
						newChromosomeMatrix[i][j] = tmp;
					}
				}
			}
			return newChromosomeMatrix;
		...
		{
			string[][] newChromosomeMatrix = new string[chromosomeNum][];
			double[][] newChromosomeMatrix2 = new double[chromosomeNum][];
			Random random = new Random();
			for (var chromosomeIndex = 0; chromosomeIndex < chromosomeNum; chromosomeIndex++)
			{
				newChromosomeMatrix[chromosomeIndex] = new string[K];
				newChromosomeMatrix2[chromosomeIndex] = new double[K];
				for (var i = 0; i < K; i++)
				{
					newChromosomeMatrix[chromosomeIndex][i] = RandomBinaryString(numberOfBits);
					newChromosomeMatrix2[chromosomeIndex][i] = Decode(newChromosomeMatrix[chromosomeIndex][i]);
				}
			}
			chromosomeMatrix = newChromosomeMatrix;
			chromosomeMatrix2 = newChromosomeMatrix2;
		}
